...
<form action="{{Route('Perfiles.update',$coop->id)}}" method="POST" >
    @csrf
    @method('PUT')
<div class="col-md-12">
    <div class="card ">
        <div class="card-header">
            <div class="row">
                <div class="col-8">
                    <h4 class="card-title" style="font-size: 16px !important; font-weight: bold !important;"><b>EDITAR COOPERATIVA</b></h4>
                </div>
    ...
    <div class="form-row">
            <div class="col-sm-5">
                <input type="text" name="descripcion" style="font-size: 14px;"  value="{{$coop->descripcion}}" id="descr" class="form-control" required autofocus  oninput="let p=this.selectionStart;this.value=this.value.toUpperCase();this.setSelectionRange(p, p);" placeholder="Descripcion">
            </div>
            {{-- <div class="col-sm-5">
                <input type="date" id="fech" value="{{$coop->fecha}}" name="fecha" class="form-control"  >
            </div> --}}
        </div>
        <br>
        @php
...
<form action="{{route('Perfiles.destroy',$coop->id)}}" id="deleeperfil" method="POST">
    @csrf
    @method('DELETE');
